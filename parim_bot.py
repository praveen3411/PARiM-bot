...
async def login(page):
    ...
    log("Login page loaded, attempting to log in...")
    ...
    # Step 2: Try clicking Next/Continue if it's a two-step login
    ...
    # Step 3: Fill password field (wait longer in case it animates in)
    try:
        pw_input = await page.wait_for_selector('input[type="password"]', timeout=15000)
        await pw_input.fill(PASSWORD)
        log("Password entered")
    except Exception:
        # Try pressing Enter on email field to trigger password field
        await email_input.press("Enter")
        await page.wait_for_timeout(2000)
        pw_input = await page.wait_for_selector('input[type="password"]', timeout=10000)
        await pw_input.fill(PASSWORD)
        log("Password entered (after Enter)")
